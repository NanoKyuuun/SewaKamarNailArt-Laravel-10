                    ...
                    <table class="w-full text-sm text-left rtl:text-right">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b ">
                                ...
                                <td class="px-6 py-4">{{ $data->kamar->name }}</td>
                                <td class="px-6 py-4">{{$data->kamar->tipe}}</td>
                                <td class="px-6 py-4">{{$data->tangal_pesan}}</td>
                                <td class="px-6 py-4">{{$data->tanggl_masuk}}</td>
                                <td class="px-6 py-4">{{$data->status_pembayaran}}</td>
                                <td class="px-6 py-4">
                                    <div class="flex">
                                        <!-- Tag <a> -->
                                        <a href="/dashboard/pemesanan/{{ $data->id }}/edit" class="mr-2">
                                        ...
                                        <!-- Tag <form> -->
                                        <form action="/dashboard/pemesanan/{{ $data->id }}" method="POST">
                                            @method('delete')
                                            @csrf
                                        ...
                                        @if ($data->pembayaran)
                                            <a href="/dashboard/pembayaran" class="mx-2">
                                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-wides">Lihat Pembayaran</button>
                                            </a>
                                        @else
                                            <a href="/dashboard/pembayaran/{{ $data->id }}" class="mx-2">
                                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-wides">Bayar</button>
                                            </a>
                                        @endif
                                    </div>
                                </td>
